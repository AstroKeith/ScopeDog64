...
def scopedog_loop(): # run at 1Hz
    global ina, lowBattery, az_Joy, alt_rate, az_rate, jStickSpeed, scopeAz, scopeAlt, alt_Joy, go_to, moving, loop, arr,ampHour, seestar_busy
    blink = True
    lowBattery = False
    ampHour = 0
    ...
    while True:
        ...
        if go_to == True:
            ...
            loop = ""
            performSSgoto()
            if param["Goto++_mode"] == '1' and rateInd != 1 and go_to == True:
                loop = "++"
                time.sleep(0.1)
        ...
        elif (x == 0 and y == 3 and ina == True): # battery screen
...
def stop_goto():
    ...
    AltStepper.setAcceleration(altAccel[2])
...
def moveScope(moveString):
    ...
    if (Az):
        ...
        if (speed == 0):  # this means stop!
            AltStepper.setAcceleration(altAccel[3])
            AltStepper.setVelocityLimit(0)
...
def do_align():
    ...
    solveImage()
    if solve == False:
        handpad.display(arr[x, y][0], "Solved Failed", "")
        return
...
def align():
    global aligning, align_count, arr, x, y
    if nexus.is_aligned() == False: # need to do the 2* alignment procedure.
        #if x == 0 and y == 0: #got here from home screen
        x = 0
        y = 2
        aligning = True
...
def access_Seestar():
    ...
    if seestar_access != True: # lets get the Seestar ready
        ...
        seestar = seestar_scopedog.Seestar(handpad,lat,lon,param['Seestar_ssid'],param['Seestar_password'])
        seestar_access = True
